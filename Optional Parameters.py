# Не обязательные параметры в функциях Python
# При определении функции вы можете указать значение по умолчанию для параметра. Затем этот параметр становится необязательным параметром при вызове функции. 
...
def test(num, bools = True, dict1 = {2:3, 4:5, 6:8}):
    if bools == True:
        for n in dict1:
            if n == num:
                return dict1[n]
...
# The second is an optional parameter called direction with a default value of True. The third is an optional parameter called d that has a default value 
# of {'apple': 2, 'pear': 1, 'fruit': 19, 'orange': 5, 'banana': 3, 'grapes': 2, 'watermelon': 7}. Write the function checkingIfIn so that when the second 
# parameter is True, it checks to see if the first parameter is a key in the third parameter; if it is, return True, otherwise return False.
...
def checkingIfIn(a, direction = True, d = {'apple': 2, 'pear': 1, 'fruit': 19, 'orange': 5, 'banana': 3, 'grapes': 2, 'watermelon': 7}):
    if direction == True:
        ...
        if a not in d:
            ...
            return False
checkingIfIn('grapes', False)    

# We have provided the function checkingIfIn such that if the first input parameter is in the third, dictionary, input parameter, then the function returns 
# that value, and otherwise, it returns False. Follow the instructions in the active code window for specific variable assignmemts.
